...
public class FFuncionario_Rol_Bodega {
    ...
    lstpar.add(new Parametro(1,funrb.getNumero_cedula().getCedula()));
    lstpar.add(new Parametro(2,funrb.getId_bodega().getId_bodega()));
    ...
    public static ArrayList<Funcionario_Rol_Bodega> llenarFuncionario_Rol_Bodega(ConjuntoResultado crs) throws Exception {
        ...
        try {
            while (crs.next()) {
              dls = new Funcionario_Rol_Bodega(FFuncionario.obtenerFuncionario_xcodigo(crs.getString(0)),FBodega.obtenerBodega_xcodigo(crs.getInt(1)),FRol.obtenerRoles_xcodigo(crs.getInt(2)),crs.getDate(3),crs.getDate(4));
